...
describe('Lean package to TypeScript module tree', () => {
  ...
  /**
   * Everything the Lean sources decide has to match exactly: the module bodies, the source maps,
   * the semantic IR, the module layout, the spans and the closure. The semantic input closure is
   * invocation-independent — `runtime-provenance.ts` resolves `src/lean-to-typescript/*.ts` from
   * the package root, so a run from source and the release run from `dist/*.js` record the same
   * digests — but it is not compared here: whether a committed tree's recorded pins are still
   * fresh is a separate property, and `bun run lean-to-typescript:package:check` proves it by
   * regenerating against the same compiler build that wrote the tree, byte for byte including the
   * provenance header.
   */
  test(

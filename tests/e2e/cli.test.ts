...
function run(fixture: string, extra: readonly string[] = []): string {
  ...
  runCli(['ts-to-lean', path.join(FIX, fixture), '-o', out, ...extra]);
...
describe('CLI e2e: generics/discriminated-unions.ts', () => {
  let code: string;
  beforeAll(() => {
    code = run('generics/discriminated-unions.ts');
  });
...
describe('CLI e2e: generics/branded-types.ts', () => {
  let code: string;
  beforeAll(() => {
    code = run('generics/branded-types.ts');
  });
...
describe('CLI e2e: advanced/for-loops.ts', () => {
  let code: string;
  beforeAll(() => {
    code = run('advanced/for-loops.ts');
  });
...
describe('CLI e2e: advanced/optional-chaining.ts', () => {
  let code: string;
  beforeAll(() => {
    code = run('advanced/optional-chaining.ts');
  });
...
describe('CLI e2e: advanced/template-literals.ts', () => {
  let code: string;
  beforeAll(() => {
    code = run('advanced/template-literals.ts');
  });
...
describe('CLI e2e: advanced/class-features.ts', () => {
  let code: string;
  beforeAll(() => {
    code = run('advanced/class-features.ts');
  });

...
def RewriteCtx.detectDiscriminant (self : RewriteCtxState) (scrutinee : String) : StateT RewriteCtxState IO (Option String) :=
  do
      ...
      if !(sorry) then
        ...
        do
          if default == "TypeRef" then
            let union := Option.getD (sorry) none
          else do
...
def RewriteCtx.rewriteDiscCase (self : RewriteCtxState) (c : String) (union : UnionInfo) (scrutineeName : Option String) : String :=
  ...
  match key with
    | none => ()
    | some _v => let info : Option VariantInfo := sorry
    if info then
      let args : Array __object := Array.map (fun f => { tag := "PVar", name := f }) info.fields
      let subst := AssocMap.empty
      let rewrittenBody := match scrutineeName with

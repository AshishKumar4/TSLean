...
One row per runtime opcode the exporter spends. `Code.kind` is the wire spelling
`src/lean-to-typescript/ir.ts` decodes, `Code.requires` is the ordered assumption closure the row's
theorem consumes, and `Code.Preserves` is the theorem's statement: the source operation on the left,
the runtime form on the right.

`registry` is a total function on `Code`. Adding a constructor without a theorem does not compile, so
...
def Code.emittedForm : Code → String
  | .boolAnd => "left && right"
  | .boolOr => "left || right"
  | .boolNot => "!operand"
...
def Code.theoremName : Code → String
...
def Code.modelField : Code → String
  | .boolAnd => "boolAnd"
  | .boolOr => "boolOr"
  | .boolNot => "boolNot"
...
def Code.runtimeSymbol : Code → String
  | .boolAnd => "&&"
  | .boolOr => "||"
  | .boolNot => "!"
  | .boolEquals => "==="
...
def Code.components : Code → List String
...
theorem Code.mem_all (code : Code) : code ∈ Code.all := by
  cases code <;> simp [Code.all]

/-- Distinct opcodes have distinct wire spellings, so the registry join is a bijection. -/
theorem Code.kind_injective {left right : Code} (equal : left.kind = right.kind) : left = right := by
  cases left <;> cases right <;> simp_all [Code.kind]

/-- The ordered assumption closure the opcode's theorem consumes. -/
def Code.requires : Code → List Assumption.Id
...
-/
def Code.Preserves (runtime : Runtime) : Code → Prop
...
def Code.Obligation (runtime : Runtime) (code : Code) : Prop :=
...
theorem boolAndModelsAnd (runtime : Runtime)
    ...
    Code.boolAnd.Preserves runtime := by
  intro left right
...
theorem boolOrModelsOr (runtime : Runtime)
    (holds : Assumption.Holds runtime Code.boolOr.requires) :
    Code.boolOr.Preserves runtime := by
  intro left right
...
theorem boolNotModelsNot (runtime : Runtime)
    (holds : Assumption.Holds runtime Code.boolNot.requires) :
    Code.boolNot.Preserves runtime := by
  intro operand
...
theorem boolEqualsModelsBEq (runtime : Runtime)
    (holds : Assumption.Holds runtime Code.boolEquals.requires) :
    Code.boolEquals.Preserves runtime := by
  intro left right
  rw [holds.1.1 (Encode.bool left) (Encode.bool right)]
...
theorem natAddModelsAdd (runtime : Runtime)
    (holds : Assumption.Holds runtime Code.natAdd.requires) :
    Code.natAdd.Preserves runtime := by
  intro left right
  unfold Encode.nat
...
theorem natSubtractModelsSub (runtime : Runtime)
    (holds : Assumption.Holds runtime Code.natSubtract.requires) :
    Code.natSubtract.Preserves runtime := by
  intro left right
...
theorem natMultiplyModelsMul (runtime : Runtime)
    (holds : Assumption.Holds runtime Code.natMultiply.requires) :
    Code.natMultiply.Preserves runtime := by
  intro left right
  unfold Encode.nat
...
theorem natLessModelsLt (runtime : Runtime)
    (holds : Assumption.Holds runtime Code.natLess.requires) :
    Code.natLess.Preserves runtime := by
  intro left right
  unfold Encode.nat
  rw [holds.1.1 (left : Int) (right : Int)]
  congr 1
...
theorem natLessOrEqualModelsLe (runtime : Runtime)
    (holds : Assumption.Holds runtime Code.natLessOrEqual.requires) :
    Code.natLessOrEqual.Preserves runtime := by
  intro left right
  unfold Encode.nat
...
theorem natEqualsModelsBEq (runtime : Runtime)
    (holds : Assumption.Holds runtime Code.natEquals.requires) :
    Code.natEquals.Preserves runtime := by
  intro left right
...
theorem natSuccessorModelsSucc (runtime : Runtime)
    (holds : Assumption.Holds runtime Code.natSuccessor.requires) :
    Code.natSuccessor.Preserves runtime := by
  intro operand
  unfold Encode.nat
...
theorem stringAppendModelsAppend (runtime : Runtime)
    (holds : Assumption.Holds runtime Code.stringAppend.requires) :
    Code.stringAppend.Preserves runtime := by
  intro left right
...
theorem stringEqualsModelsBEq (runtime : Runtime)
    (holds : Assumption.Holds runtime Code.stringEquals.requires) :
    Code.stringEquals.Preserves runtime := by
  intro left right
...
theorem listLengthModelsLength (runtime : Runtime)
    (holds : Assumption.Holds runtime Code.listLength.requires) :
    Code.listLength.Preserves runtime := by
...
theorem listIsEmptyModelsIsEmpty (runtime : Runtime)
    (holds : Assumption.Holds runtime Code.listIsEmpty.requires) :
    Code.listIsEmpty.Preserves runtime := by
  intro α encode values
...
theorem listAppendModelsAppend (runtime : Runtime)
    (holds : Assumption.Holds runtime Code.listAppend.requires) :
    Code.listAppend.Preserves runtime := by
...
theorem listReverseModelsReverse (runtime : Runtime)
    (holds : Assumption.Holds runtime Code.listReverse.requires) :
    Code.listReverse.Preserves runtime := by
  intro α encode values
...
theorem listMapModelsMap (runtime : Runtime)
    (holds : Assumption.Holds runtime Code.listMap.requires) :
    Code.listMap.Preserves runtime := by
...
theorem listFilterModelsFilter (runtime : Runtime)
    (holds : Assumption.Holds runtime Code.listFilter.requires) :
    Code.listFilter.Preserves runtime := by
...
theorem listFoldLeftModelsFoldl (runtime : Runtime)
    (holds : Assumption.Holds runtime Code.listFoldLeft.requires) :
    Code.listFoldLeft.Preserves runtime := by
...
theorem listFoldRightModelsFoldr (runtime : Runtime)
    (holds : Assumption.Holds runtime Code.listFoldRight.requires) :
    Code.listFoldRight.Preserves runtime := by
  intro α β element state step function callback initial values
...
theorem listAnyModelsAny (runtime : Runtime)
    (holds : Assumption.Holds runtime Code.listAny.requires) :
    Code.listAny.Preserves runtime := by
...
theorem listAllModelsAll (runtime : Runtime)
    (holds : Assumption.Holds runtime Code.listAll.requires) :
    Code.listAll.Preserves runtime := by
  intro α encode holdsCallback decision callback values
...
theorem listHeadModelsHead (runtime : Runtime)
    (holds : Assumption.Holds runtime Code.listHead.requires) :
    Code.listHead.Preserves runtime := by
  intro α encode values
...
theorem listFirstModelsHead (runtime : Runtime)
    (holds : Assumption.Holds runtime Code.listFirst.requires) :
    Code.listFirst.Preserves runtime := by
...
theorem listRestModelsTail (runtime : Runtime)
    (holds : Assumption.Holds runtime Code.listRest.requires) :
    Code.listRest.Preserves runtime := by
  intro α encode head tail
  rw [List.map_cons, holds.1.2.2.2.2.2.2.2.2.2.2 (encode head) (List.map encode tail)]

/--
The closure over the opcode registry. It is a total function on `Code`, so an opcode with no theorem
...
-/
theorem registry (runtime : Runtime) : (code : Code) → code.Obligation runtime
...
theorem registry_total (runtime : Runtime) (code : Code)

...
partial def mapType (t : Type) (checker : Any) (depth : Float := 0) : IRType :=
...
-- // ─── Union types ────────────────────────────────────────────────────────────────
def mapUnion (t : UnionType) (checker : Any) (depth : Float) : IRType :=
...
-- // ─── Intersection types ─────────────────────────────────────────────────────────
def mapIntersection (t : IntersectionType) (checker : Any) (depth : Float) : IRType :=
...
-- // ─── Object types ───────────────────────────────────────────────────────────────
def mapObject (t : ObjectType) (checker : Any) (depth : Float) : IRType :=
...
def mapTypeRef (t : TypeReference) (checker : Any) (depth : Float) : IRType :=
...
partial def typeStr (t : IRType) : String :=
  match t.tag with
    ...
    | "Tuple" => if default.size == 0 then
      ...
      ("(" ++ (String.intercalate " × " default.map typeStr)) ++ ")"
    | "Function" => let paramStr : String := if default.size == 0 then
      "Unit"
    else
      String.intercalate " → " default.map (fun p => irTypeToLean p true)
    ...
    if tsOnlyTypes.has default then
      "Any"
    else
      if default.size == 0 then
        default
      else
        (s!"{default} ") ++ (String.intercalate " " default.map (fun a => irTypeToLean a true))
    | "TypeVar" => default
    | "Structure" => default
    | "Inductive" => default

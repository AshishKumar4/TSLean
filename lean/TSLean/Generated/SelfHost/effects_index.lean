...
def inferNodeEffect (node : Any) (checker : Any) : Effect :=
...
def monadString (effect : Effect) (stateTypeName : String := "σ") : String :=
  match effect.tag with
    ...
    if se then
      parts.push ("StateT " ++ (leanTypeName default))
    else
      if default.any (fun e => e.tag == "State") then
        ...
        ()
    if ee then
      parts.push ("ExceptT " ++ (leanTypeName default))
    else
...
partial def leanTypeName (t : IRType) : String :=
  match t.tag with
    ...
    | "TypeRef" => if default.size == 0 then
      ...
      ((s!"({default} ") ++ (String.intercalate " " default.map leanTypeName)) ++ ")"

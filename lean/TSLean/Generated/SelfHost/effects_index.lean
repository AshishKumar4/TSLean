...
def monadString (effect : Effect) (stateTypeName : String := "σ") : String :=
  match effect.tag with
    ...
    if se then
      ...
      if effect.effects.any (fun e => e.tag == "State") then
    ...
    if ee then
      ...
      if effect.effects.any (fun e => e.tag == "Except") then
  ...
  if b.tag == "Pure" then
    ...
    if a.tag == b.tag then
      ...
      if a.tag == "Combined" then
        a.effects.any (fun e => effectSubsumes e b)
      else
        false
  ...
  if ((ts.isFunctionDeclaration node) || (ts.isFunctionExpression node)) || (ts.isMethodDeclaration node) then
    ...
    if ts.isArrowFunction node then
      ...
      if ts.isVariableStatement node then
        ...
        if (d.bind (fun _oc => _oc.initializer)) && ((ts.isArrowFunction d.initializer) || (ts.isFunctionExpression d.initializer)) then
...
partial def bodyContainsIO (node : Node) : Bool :=
  if ts.isCallExpression node then
    ...
    if (IO_TRIGGERING_PREFIXES.any (fun p => text.startsWith p)) || (IO_TRIGGERING_CALLS.has text) then
      true
    else
      ()
  else
...
partial def leanTypeName (t : IRType) : String :=
  match t.tag with
    ...
    | "TypeRef" => if t.args.size == 0 then
      ...
      ((s!"({t.name} ") ++ (t.args.map leanTypeName.foldl (· ++ ·) "" " ")) ++ ")"

...
export interface BehaviourRequest extends LeanCheckOptions {
  ...
  /**
   * How many declared parameters the Lean type former behind each profile type name erased.
   *
   * Empty for the recovered side, where the former is already unparameterised. The source side
   * elaborates against the original module, where a phantom-indexed former still takes its index,
   * so the driver has to abstract over it. Read from the manifest, which records it, rather than
   * guessed from the generated bytes, which cannot show it.
   */
  readonly erasedIndices?: ReadonlyMap<string, number>;
...
export async function compareBehaviour(request: BehaviourRequest): Promise<BehaviourReport> {
  ...
  const lean = evaluateLean(
    request.lean,
    request.leanImports ?? [],
    plans,
    request.typeModules,
    request.erasedIndices ?? new Map(),
    request,
  );
...
function evaluateLean(
  ...
  erasedIndices: ReadonlyMap<string, number>,
  ...
  const run = leanRun(modules, {
    ...
    code: driverSource([...modules.map((entry) => entry.module), ...provided], plans, typeModules, erasedIndices),
...
function driverSource(
  ...
  /** The binder a definition gives one erased index. Positional, so `applied` and `binders` agree. */
  const indexBinder = (position: number): string => `tsleanIndex${String(position)}`;
  const indexCount = (name: string): number => erasedIndices.get(name) ?? 0;
  const naming: TypeNaming = {
    applied: (name) => {
      const count = indexCount(name);
      if (count === 0) return qualify(name);
      return [qualify(name), ...Array.from({ length: count }, (_, at) => indexBinder(at))].join(' ');
    },
    binders: (type) => {
      const target = type.kind === 'option' ? type.inner : type;
      if (target.kind !== 'structure' && target.kind !== 'enumeration') return '';
      const count = indexCount(target.name);
      // The binder type is a hole: Lean reads it off the type former's own signature, so the
      // driver never needs the index's type, only how many there are.
      return Array.from({ length: count }, (_, at) => ` {${indexBinder(at)} : _}`).join('');
    },
    indexed: (name) => indexCount(name) > 0,
  };
  const renderers = new Map<string, string>();
  const rows: string[] = [];

  for (const [index, plan] of plans.entries()) {
    collectRenderers(plan.observed.result, naming, renderers);
    rows.push(`def ${ROWS}${String(index)} : List String :=\n  ${application(plan, naming)}`);
  }
 ...
 */
function application(plan: ObservationPlan, naming: TypeNaming): string {
  ...
  const arguments_ = plan.observed.parameters.map((type, position) => {
    ...
    return leanValueAt(type, `((index / ${String(stride)}) % ${String(size)})`, naming);
  });
...
function leanValueAt(
  ...
  if (type.kind === 'option') {
    return `(match ${index} with | 0 => none | _ => some ${leanValueAt(type.inner, `(${index} - 1)`, naming)})`;
  }
  const fields = type.fields.map((field, position) => {
    ...
    return `${field.name} := ${leanValueAt(field.type, fieldIndex, naming)}`;
  });
  // A phantom-indexed type's value is written without an ascription. Every position the driver
  // splices one into — an argument of the observed function, a field of a structure literal, the
  // operand of `some` under a known result type — already fixes the expected type, so Lean
  // determines the index from the surrounding elaboration. Ascribing it would demand the index
  // here, where the driver does not know which one the use site meant.
  if (naming.indexed(type.name)) return `({ ${fields.join(', ')} })`;
  return `({ ${fields.join(', ')} } : ${naming.applied(type.name)})`;
}
...
/**
 * How a source type is written in the driver, and what a definition mentioning it has to abstract
 * over.
 *
 * A Lean type former with a term index is emitted unparameterised — `Label (tag : Tag)` becomes
 * `interface Label` — so the profile, which is read off the TypeScript, knows nothing about the
 * index. The driver elaborates against the *original* Lean module, where the former still takes
 * its index, so it has to put one back. `applied` writes `Label tsleanIndex0`; `binders` supplies
 * the `{tsleanIndex0 : _}` that binds it. The binder's type is a hole because Lean infers it from
 * the former's own signature, which means the driver never has to carry the index's type.
 */
interface TypeNaming {
  /** The Lean type expression for a profile type name, with any erased index applied. */
  readonly applied: (name: string) => string;
  /** The implicit binders a definition over this type takes, or the empty string. */
  readonly binders: (type: ProfileType) => string;
  /** Whether a structure's Lean type takes an erased index, so a value of it cannot be ascribed. */
  readonly indexed: (name: string) => boolean;
}
...
function collectRenderers(
  ...
  if (type.kind === 'enumeration') {
    ...
    into.set(name, [`def ${name} (value : ${naming.applied(type.name)}) : String := match value with`, ...arms].join('\n'));
    return;
  }
  if (type.kind === 'option') {
    collectRenderers(type.inner, naming, into);
    into.set(name, [
      `def ${name}${naming.binders(type)} (value : Option (${leanType(type.inner, naming.applied)})) : String := match value with`,
      '  | none => "none"',
      `  | some inner => "some(" ++ ${rendererName(type.inner)} inner ++ ")"`,
    ].join('\n'));
    return;
  }
  for (const field of type.fields) collectRenderers(field.type, naming, into);
  const parts = type.fields.map((field) => `${rendererName(field.type)} value.${field.name}`);
  into.set(name, [
    `def ${name}${naming.binders(type)} (value : ${naming.applied(type.name)}) : String :=`,
    `  "{" ++ ${parts.join(' ++ "," ++ ')} ++ "}"`,
  ].join('\n'));
}
